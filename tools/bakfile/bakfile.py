...
def try_get_title(url):
    ...
    url_full, status_code, title, content_length = url, 0, 0, ''
    try:
        ...
        pass
    return url_full, status_code, title, content_length
...
def run(url):
    ...
    url_full, status_code, title, content_length = try_get_title(url)
    if status_code != 0:
        ...
        for url_full, status_code, title, content_length in pool.map(try_get_title, urls):
            if title:
                info = {}
