...
def run(url, xray=False):
    ...
    radcmd = [os.path.join(FILEPATH, 'rad_linux_amd64'),
              "-t", url, "--json-output", out_file_name]
    ...
    if os.path.exists(out_file_name):
        for line in open(out_file_name, 'r', encoding='utf8').readlines():
            try:
                ...
                result.append({'url': rj['URL'], 'method': rj['Method'],
                              'header': rj['Header'], 'b64_body': rj.get('b64_body', '')})
